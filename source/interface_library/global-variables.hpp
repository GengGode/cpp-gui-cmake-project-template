...
namespace global
{
    namespace _detail
    {
        ...
        private:
            static uint64_t cast(const T* instance) { return reinterpret_cast<uint64_t>(instance); }
        ...
        public:
            static std::shared_ptr<T> get(const uint64_t id)
            {
                std::lock_guard<std::mutex> lock(instance_mutex);
            ...
            static std::shared_ptr<T> get(const T* instance) { return get(cast(instance)); }
            template <typename U> static std::shared_ptr<T> get(const U* instance)
            {
                static_assert(std::is_base_of_v<T, U>, "U must be derived from T");
                return get(cast(static_cast<const T*>(instance)));
            }
            template <typename U> static std::shared_ptr<T> get(const U* instance)
            {
                static_assert(std::is_base_of_v<U, T>, "U must be base of T");
                return get(cast(dynamic_cast<const T*>(instance)));
            }

            static void set(const std::shared_ptr<T>& instance)
            {
                std::lock_guard<std::mutex> lock(instance_mutex);
                instances[cast(instance)] = instance;
            }
            static void set(const std::shared_ptr<T>& instance, const uint64_t id)
            {
                std::lock_guard<std::mutex> lock(instance_mutex);
            ...
            {
                auto instance = std::make_shared<T>(std::forward<Args>(args)...);
                return { cast(instance), instance };
            }

            static void destroy(const uint64_t id)
            {
                std::lock_guard<std::mutex> lock(instance_mutex);
                instances.erase(id);
            }
            static void destroy(const T* instance) { destroy(cast(instance)); }
            template <typename U> static void destroy(const U* instance)
            {
                static_assert(std::is_base_of_v<T, U>, "U must be derived from T");
                destroy(cast(static_cast<const T*>(instance)));
            }
            template <typename U> static void destroy(const U* instance)
            {
                static_assert(std::is_base_of_v<U, T>, "U must be base of T");
                destroy(cast(dynamic_cast<const T*>(instance)));
            }
    ...
    namespace onlyone
    {
        namespace _detail
        {
            ...
            public:
                ...
                template <typename... Args> static std::shared_ptr<T> get_or_create(Args&&... args)
                {
                    std::lock_guard<std::mutex> lock(instance_mutex);
                    if (!instance)
                ...
                static void set(const std::shared_ptr<T>& inst)
                {
                    std::lock_guard<std::mutex> lock(instance_mutex);
                    instance = inst;
                }
